...
## V3.1.0 (2026-05-24) - CNStock strategy paper trading

### New Features

- Added `execution_mode='paper'` as a first-class strategy execution mode.
- Added A-share (`CNStock`) strategy paper trading: strategies can run before real trading, produce local paper fills, and maintain simulated positions/trades through the existing strategy portfolio APIs.
- CNStock paper fills are conservative by design: long-only, spot-only, buy orders rounded down to 100-share board lots, and T+1 sellability enforced.
- CNStock paper fee defaults: 0.03% commission on both sides and 0.05% stamp tax on sells. `trading_config` may override `commission`, `paper_commission`, `stamp_tax` / `paper_stamp_tax`, and `slippage`.

### Backend / API

- Strategy create/update now accepts `execution_mode: "paper"` with `market_category: "CNStock"`, `market_type: "spot"`, and `trade_direction: "long"`.
- `CNStock + live` remains rejected: A-share real broker execution is not implemented.
- `GET /api/strategies/positions` now refreshes prices using the strategy's own `market_category` instead of always routing through Crypto data.
- Paper strategy signals still write `pending_orders` for audit, while simulated fills reuse `qd_strategy_trades` and `qd_strategy_positions`.
- `GET /api/policy/broker-market` now includes `paper_market_rules.CNStock` so QuantDinger-Vue can drive the UI without duplicating backend rules.

### Frontend Follow-up

- QuantDinger-Vue should add "Paper" to strategy execution mode controls.
- When market is CNStock, default to Paper, disable Live, hide broker credential selection, force long-only, and show the A-share paper rules.
- Strategy detail views can reuse existing positions/trades APIs and label the account as paper/simulated.
- Concrete implementation notes are available at `docs/FRONTEND_CN_STOCK_PAPER_TRADING.md`.
