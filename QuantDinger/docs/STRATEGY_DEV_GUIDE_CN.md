...
`CNStock` 策略现在可以使用 `execution_mode='paper'` 做实际交易前的模拟运行。该模式不会连接真实券商，也不会发送真实订单；成交、持仓和盈亏会写入现有策略成交/持仓接口。

- 仅支持做多，`trade_direction='long'`
- 仅支持现货语义，`market_type='spot'`
- 买入按 100 股整数手向下取整，不足 100 股拒绝成交
- 卖出遵守 T+1，当日买入数量当日不可卖
- 默认佣金 0.03%，卖出印花税 0.05%，可通过 `trading_config` 覆盖
