              ...
              <a-card :bordered="false" class="strategy-content-card">
                <a-tabs defaultActiveKey="positions">
                  <a-tab-pane key="positions" :tab="$t('trading-assistant.tabs.positions')">
                    <a-alert
                      v-if="selectedStrategyIsCnStockPaper"
                      type="info"
                      show-icon
                      class="section-inline-alert simulated-strategy-alert"
                      :message="$t('trading-assistant.paper.cnStockDetailTitle') || 'CNStock paper strategy'"
                  ...
                  <a-tab-pane key="trades" :tab="$t('trading-assistant.tabs.tradingRecords')">
                    <a-alert
                      ...
                      show-icon
                      class="section-inline-alert simulated-strategy-alert"
                      :message="$t('trading-assistant.paper.simulatedRecords') || 'Simulated records'"
                      :description="cnStockPaperRuleText" />
                    <trading-records
                      :strategy-id="selectedStrategy.id"
                      :market-category="selectedStrategy.market_category || 'Crypto'"
                        ...
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                          <a-form-item :label="$t('trading-assistant.form.marketType')">
                            <a-radio-group
                              ...
                              <a-radio value="swap" :disabled="isAlpacaCryptoSpotOnly || isCNStockMarket">{{ $t('trading-assistant.form.marketTypeFutures') }}</a-radio>
                            ...
                            <div
                              v-if="isCNStockMarket"
                              class="form-item-hint"
                              style="color: #ff9800;">
                              {{ cnStockPaperRuleText }}
                            </div>
                          </a-form-item>
                        </a-col>
                      </a-row>
                        ...
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                          <a-form-item :label="$t('trading-assistant.form.tradeDirection')">
                            <a-radio-group
                              ...
                              :disabled="form.getFieldValue('market_type') === 'spot' || isLongOnlyBroker || isCNStockMarket">
                  ...
                  <a-form :form="form" layout="vertical">
                    ...
                    <a-form-item :label="$t('trading-assistant.form.tradeDirection')">
                      <a-select
                        ...
                        :disabled="isLongOnlyBroker || isCNStockMarket"
                        :getPopupContainer="(triggerNode) => triggerNode.parentNode">
                    ...
                    <div class="execution-section-card">
                      ...
                      <a-form-item :label="$t('trading-assistant.form.executionMode')" class="compact-form-item">
                        ...
                        <div class="execution-mode-cards">
                          ...
                          <div
                            :class="['execution-mode-card', 'paper-card', { active: executionModeUi === 'paper', disabled: !canUsePaperTrading }]"
                            @click="canUsePaperTrading && setExecutionModeUi('paper')">
                            <div class="execution-mode-card-icon paper">
                              <a-icon type="experiment" />
                            </div>
                            <div class="execution-mode-card-body">
                              <div class="execution-mode-card-title">{{ $t('trading-assistant.form.executionModePaper') || 'Paper' }}</div>
                              <div class="execution-mode-card-desc">
                                {{ isCNStockMarket ? cnStockPaperRuleText : ($t('trading-assistant.form.executionModePaperDesc') || 'Simulated execution without real broker orders.') }}
                              </div>
                            </div>
                            <a-icon v-if="executionModeUi === 'paper'" type="check-circle" theme="filled" class="execution-mode-card-check" />
                          </div>
                        ...
                        <div v-if="!canUseLiveTrading" class="form-item-hint execution-warn-text">
                          {{ isCNStockMarket ? cnStockPaperRuleText : $t('trading-assistant.form.liveTradingNotSupportedHint') }}
                        </div>
                      </a-form-item>
...
export default {
  ...
  computed: {
    ...
    isCNStockMarket () {
      return this.selectedMarketCategory === 'CNStock'
    },
    canUsePaperTrading () {
      const cat = this.selectedMarketCategory || 'Crypto'
      const paperMarkets = (this.brokerMarketPolicy.paper_market_categories || [])
        .map(s => String(s))
      return paperMarkets.includes(cat)
    },
    cnStockPaperRule () {
      return ((this.brokerMarketPolicy.paper_market_rules || {}).CNStock) || {}
    },
    cnStockPaperRuleText () {
      const lotSize = this.cnStockPaperRule.lot_size || 100
      return this.$t('trading-assistant.paper.cnStockRuleHint', { lotSize }) ||
        `Paper only: long-only, ${lotSize}-share board lots, T+1 sells, no real broker execution.`
    },
    selectedStrategyIsCnStockPaper () {
      const s = this.selectedStrategy || {}
      return s.market_category === 'CNStock' && (s.execution_mode || '') === 'paper'
    },
  ...
  watch: {
    ...
    selectedMarketCategory () {
      this.applyMarketExecutionDefaults()
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    applyMarketExecutionDefaults () {
      if (!this.form || !this.form.setFieldsValue) return
      if (this.selectedMarketCategory !== 'CNStock') return

      const rule = this.cnStockPaperRule || {}
      const patch = {
        execution_mode: rule.execution_mode || 'paper',
        market_type: rule.market_type || 'spot',
        trade_direction: rule.trade_direction || 'long',
        leverage: 1,
        credential_id: undefined
      }
      this.executionModeUi = patch.execution_mode
      this.currentExchangeId = ''
      this.currentBrokerId = ''
      try {
        this.form.setFieldsValue(patch)
      } catch (e) { }
    },
    handleWatchlistSymbolChange (val) {
      ...
      // Markets without live/paper trading support: force back to signal mode.
      const supportsLiveTrading = ['Crypto', 'USStock', 'Forex'].includes(this.selectedMarketCategory)
      if (this.selectedMarketCategory === 'CNStock') {
        this.executionModeUi = 'paper'
        try {
          this.form && this.form.setFieldsValue && this.form.setFieldsValue({ execution_mode: 'paper' })
        } catch (e) { }
      } else if (!supportsLiveTrading) {
        this.executionModeUi = this.selectedMarketCategory === 'CNStock' ? 'paper' : 'signal'
        try {
          this.form && this.form.setFieldsValue && this.form.setFieldsValue({ execution_mode: this.executionModeUi })
        } catch (e) { }
      }
    ...
    onExecutionModeChange (e) {
      ...
      if (this.isCNStockMarket && this.executionModeUi !== 'paper') {
        this.executionModeUi = 'paper'
        try {
          this.form && this.form.setFieldsValue && this.form.setFieldsValue({ execution_mode: 'paper' })
        } catch (err) { }
      ...
      // If market doesn't support live trading, force signal mode unless paper is allowed.
      if (!this.canUseLiveTrading && this.executionModeUi !== 'signal' && !(this.canUsePaperTrading && this.executionModeUi === 'paper')) {
    ...
    handleMarketTypeChange (e) {
      ...
      if (this.isCNStockMarket && marketType !== 'spot') {
        this.applyMarketExecutionDefaults()
        return
      }
        ...
        if (!err) {
          try {
            ...
            if (this.strategyMode === 'script') {
              ...
              if (marketCategory === 'CNStock') {
                marketType = this.cnStockPaperRule.market_type || 'spot'
                leverage = 1
                tradeDirection = this.cnStockPaperRule.trade_direction || 'long'
              }
              ...
              const payload = {
                ...
                execution_mode: marketCategory === 'CNStock' ? 'paper' : (values.execution_mode || 'signal'),
            ...
            const selectedSymbolMarket = (!this.isEditMode && Array.isArray(this.selectedSymbols) && this.selectedSymbols[0] && String(this.selectedSymbols[0]).includes(':'))
              ? String(this.selectedSymbols[0]).split(':', 1)[0]
              : this.selectedMarketCategory
            const effectiveMarketCategory = selectedSymbolMarket || this.selectedMarketCategory || 'Crypto'
            const effectiveExecutionMode = effectiveMarketCategory === 'CNStock'
              ? 'paper'
              : (values.execution_mode || 'signal')
            const isLive = this.canUseLiveTrading && effectiveExecutionMode === 'live'
            ...
            if (effectiveMarketCategory === 'CNStock') {
              marketType = this.cnStockPaperRule.market_type || 'spot'
              leverage = 1
              tradeDirection = this.cnStockPaperRule.trade_direction || 'long'
            }
            ...
            const basePayload = {
              strategy_name: values.strategy_name,
              market_category: effectiveMarketCategory,
            ...
            if (this.editingStrategy) {
              ...
              if (basePayload.market_category === 'CNStock') {
                basePayload.execution_mode = 'paper'
                basePayload.exchange_config = undefined
                basePayload.trading_config.market_type = this.cnStockPaperRule.market_type || 'spot'
                basePayload.trading_config.leverage = 1
                basePayload.trading_config.trade_direction = this.cnStockPaperRule.trade_direction || 'long'
              }
