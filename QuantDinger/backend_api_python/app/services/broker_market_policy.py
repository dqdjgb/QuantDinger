...
# Markets that can run through QuantDinger's local paper-fill path. This does
# not imply live broker support.
PAPER_MARKET_CATEGORIES: Set[str] = {"Crypto", "USStock", "Forex", "CNStock"}

PAPER_MARKET_RULES: Dict[str, Dict[str, object]] = {
    "CNStock": {
        ...
        "frontend_hint": (
            "CNStock supports paper strategy trading only. Disable live "
            "broker execution, force long-only spot mode, and show T+1 / "
            "100-share lot rules."
        ),
    }
}
...
def validate_strategy_config(
    *,
    ...
    mode = (execution_mode or ("live" if require_exchange else "signal")).strip().lower()

    if mode == "paper":
        if mc and mc not in PAPER_MARKET_CATEGORIES:
            raise ValueError(
                f"market_category='{mc}' is not supported for paper trading. "
                f"Supported: {sorted(PAPER_MARKET_CATEGORIES)}."
            )
        if mc == "CNStock":
            if mt and mt != "spot":
                raise ValueError("CNStock paper trading requires market_type='spot'.")
            if td and td != "long":
                raise ValueError("CNStock paper trading is long-only; set trade_direction='long'.")
            if bt and bt in BOT_TYPE_MARKETS and mc not in BOT_TYPE_MARKETS[bt]:
...
def to_dict() -> Dict[str, object]:
    ...
    return {
        ...
        "paper_market_categories": sorted(PAPER_MARKET_CATEGORIES),
