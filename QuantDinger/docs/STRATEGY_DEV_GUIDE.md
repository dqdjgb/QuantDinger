...
For China A-shares (`CNStock`), use `execution_mode='paper'` before any
real-money workflow. QuantDinger does not route CNStock live broker orders in
this backend. CNStock paper trading is intentionally conservative:

- long-only and `market_type='spot'`
- buy fills are rounded down to 100-share board lots
- shares bought today cannot be sold until the next trading day (T+1)
- simulated fills are stored in the normal strategy trades and positions APIs
