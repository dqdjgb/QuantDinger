...
class TestValidateIllegalCombos:
    ...
    def test_cnstock_paper_long_allowed_without_exchange(self):
        validate_strategy_config(
            exchange_id="",
            market_category="CNStock",
            market_type="spot",
    ...
    def test_cnstock_live_rejected(self):
        with pytest.raises(ValueError, match="not supported for live trading"):
            validate_strategy_config(
                exchange_id="",
                market_category="CNStock",
    ...
    def test_cnstock_paper_short_rejected(self):
    ...
    def test_cnstock_paper_swap_rejected(self):
        with pytest.raises(ValueError, match="requires market_type='spot'"):
            validate_strategy_config(
                exchange_id="",
                market_category="CNStock",
                market_type="swap",
...
class TestToDictSnapshot:
    ...
    def test_paper_market_categories_serialized(self):
        assert sorted(to_dict()["paper_market_categories"]) == sorted(PAPER_MARKET_CATEGORIES)
        assert "CNStock" in to_dict()["paper_market_categories"]

    def test_cnstock_paper_rules_serialized_for_frontend(self):
        rules = to_dict()["paper_market_rules"]["CNStock"]
        assert rules["execution_mode"] == "paper"
        assert rules["market_type"] == "spot"
        assert rules["trade_direction"] == "long"
        assert rules["lot_size"] == 100
        assert rules["t_plus_1"] is True
        assert rules["live_supported"] is False
