...
class TradingExecutor:
    ...
    def _run_strategy_loop(self, strategy_id: int):
        ...
        try:
            ...
            if execution_mode not in ['signal', 'live', 'paper']:
    ...
    def _execute_signal(
        ...
        try:
            ...
            sig = (signal_type or "").strip().lower()
            is_cn_paper = (
                str(execution_mode or "").strip().lower() == "paper"
                and (market_category or "").strip() == "CNStock"
            )
            if is_cn_paper and not cn_paper.is_supported_signal(sig):
                append_strategy_log(
                    strategy_id,
                    "info",
            ...
            paper_commission_override = None
            if is_cn_paper:
                sellable_amount = None
                if cn_paper.is_sell_signal(sig):
                    sellable_amount = self._get_cnstock_paper_sellable_amount(strategy_id, symbol, current_positions)
                paper_fill = cn_paper.build_fill(
                    signal_type=sig,
                ...
                fill_price = paper_fill.price
                paper_commission_override = paper_fill.commission
            ...
            if order_result and order_result.get('success'):
                ...
                if _exchange_fee and _exchange_fee.get('taker', 0) > 0:
                    ...
                    if _comm_rate <= 0:
                        _comm_rate = 0.001
                _est_commission = round(float(fill_price or 0) * float(amount or 0) * _comm_rate, 8)
                if paper_commission_override is not None:
                    _est_commission = float(paper_commission_override or 0.0)
                ...
                if 'open' in sig or 'add' in sig:
                    ...
                    if old_pos:
                        ...
                        new_size += old_size
                        new_entry = ((old_size * old_entry) + (amount * fill_price)) / new_size

                    self._update_position(
                        strategy_id=strategy_id, symbol=symbol, side=side,
                        size=new_size, entry_price=new_entry, current_price=fill_price
                    )
                    _prefix = "Paper fill: " if str(execution_mode or "").strip().lower() == "paper" else ""
                    append_strategy_log(
                        strategy_id, "trade",
                    ...
                    if old_entry > 0 and amount > 0:
                        if side == 'long':
                            reduce_profit = (fill_price - old_entry) * amount
                        else:
                            reduce_profit = (old_entry - fill_price) * amount
                    ...
                    if new_size <= old_size * 0.001:
                        ...
                        self._update_position(
                            strategy_id=strategy_id, symbol=symbol, side=side,
                            size=new_size, entry_price=old_entry, current_price=fill_price
                        )
                    ...
                    if old_pos:
                        ...
                        if entry_price > 0 and amount > 0:
                            if side == 'long':
                                close_profit = (fill_price - entry_price) * amount
                            else:
                                close_profit = (entry_price - fill_price) * amount
    ...
    def _enqueue_pending_order(
        ...
        try:
            now = int(time.time())
            # Local deployment supports signal, paper, and live.
    ...
    def _get_cnstock_paper_sellable_amount(
        self,
        strategy_id: int,
        symbol: str,
        current_positions: Optional[List[Dict[str, Any]]] = None,
    ) -> float:
        """A-share paper T+1: shares bought today are not sellable today."""
        current_size = 0.0
        for pos in current_positions or []:
            if (pos.get("side") or "").strip().lower() == "long":
                try:
        ...
        try:
            with get_db_connection() as db:
                ...
                cursor.execute(
                    """
                    SELECT COALESCE(SUM(amount), 0) AS bought_today
                    FROM qd_strategy_trades
                    WHERE strategy_id = %s
                      AND symbol = %s
                      AND type IN ('open_long', 'add_long')
                ...
                row = cursor.fetchone() or {}
                cursor.close()
            bought_today = float(row.get("bought_today") or 0.0)
            return max(0.0, current_size - bought_today)
        except Exception as e:
