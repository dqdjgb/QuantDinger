...
def search_symbols():
    ...
    try:
        ...
        if market == 'Crypto' and len(out) < 3:
            ...
            out.extend(extra)
        elif market == 'CNStock' and len(out) < limit:
            from app.data_sources.cn_stock_symbols import search_a_share_symbols
            extra = search_a_share_symbols(
                keyword,
                limit - len(out),
            ...
            out.extend(extra)
            for item in extra:
                persist_seed_name('CNStock', item.get('symbol') or '', item.get('name') or '')
